        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            if (isset($_FILES["text"]) && $_FILES["text"]["error"] == 0) {
                ...
                if (in_array($filetype, $allowed)) {
                    ...
                    if (file_exists("/public/csv/" . $filename)) {
                        $isuploaded = "error: " . $filename . " already exists.";
                    } else {
                        move_uploaded_file($_FILES["text"]["tmp_name"], "/public/csv/" . $filename);
                        ...
                        if (!$con) {
                            ...
                            $sql = "LOAD DATA INFILE '/public/csv/" . $filename . "' INTO TABLE probe_data FIELDS TERMINATED BY ',' LINES TERMINATED BY '\\n' IGNORE 1 ROWS;";
